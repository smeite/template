...
import { getManagerMenuList  }  from '@/api/system/auth/manager'
export default {
  namespaced: true,
  state: {
    // 顶栏菜单
  ...
  actions: {
    ...
    async asideLoad({ state, dispatch }) {
      // store 赋值
      const menuData = await dispatch('shouwang/db/get', {
        dbName: 'sys',
        path: 'menu',
    ...
    async sourceDataLoad({ state, dispatch }) {
      // 加载菜单数据源
      state.sourceData = await dispatch('shouwang/db/get', {
        dbName: 'database',
    ...
    async historyLoad({ state, dispatch }) {
      // 获取历史菜单持久化
      state.history = await dispatch('shouwang/db/get', {
        dbName: 'database',
        path: '$menu.history',
        defaultValue: [],
        user: true
