...
function errorLog(error) {
  ...
  // 添加到日志
  store.dispatch('shouwang/log/push', {
    type: 'danger',
    info: '数据请求异常',
...
service.interceptors.request.use(
  config => {
    config.headers['X-Token'] = util.cookies.get('token')
...
service.interceptors.response.use(
  ...
  error => {
    ...
    if (!error.response) {
      ...
      error.message = message || get(error, 'response.message')
...
function refreshToken(config) {
  ...
  // 以下接口不需要刷新令牌
  const whiteList = [
    '/public/manager/refresh',
    '/api/system/manager/logout',
    '/public/manager/login'
  ]

  if (whiteList.indexOf(get(config, 'url')) >= 0) {
    return
  }

  let userInfo = store.state.shouwang.user.info
  ...
  if ((nowTime - 3600) > userInfo.token.token_expires && nowTime < userInfo.token.refresh_expires) {
    service({
      ...
      .then(res => {
        ...
        store.dispatch('shouwang/user/set', userInfo, { root: true }).then(() => {})
